...
//  Serial.print("Effect #"); Serial.println(effect);
  ...
  if (effect == 74) {
    Serial.println(F("74 − Transition Ramp Down Short Smooth 1 – 100 to 0%")); // might work as start
  }
  if (effect == 75) {
  ...
  if (effect == 82) {
    Serial.println(F("82 − Transition Ramp Up Long Smooth 1 – 0 to 100%")); // TO YOU
  }
  if (effect == 83) {
  ...
  if (effect == 94) {
    Serial.println(F("94 − Transition Ramp Down Long Smooth 1 – 50 to 0%")); END
  }
  if (effect == 95) {
  ...
  // wait a bit
  delay(3000); // modified delay to understand each vibration from 500 to 3000
